...
async def test_background_tasks_forbidden_without_scope(async_client, monkeypatch):
    ...
    for method, path in read_paths:
        r = await async_client.request(method, path, headers={"Authorization": "Bearer x"})
...
@pytest.mark.asyncio
async def test_background_tasks_pass_auth_then_celery_guard_kicks_in(async_client, monkeypatch):
    """
    ...
    for method, path in [
        ...
        r = await async_client.request(method, path, headers={"Authorization": "Bearer ok"})
        if settings_mod.ENABLE_CELERY:
