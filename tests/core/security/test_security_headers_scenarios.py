...
def test_headers_set_and_csp_varies():
    ...
    r = client.get("/docs")
    assert r.headers["Content-Security-Policy"].startswith("default-src 'self'; script-src 'self' 'unsafe-inline'")
    assert r.headers["X-Content-Type-Options"] == "nosniff"
    assert r.headers["X-Frame-Options"] == "DENY"
    assert r.headers["Referrer-Policy"] == "strict-origin-when-cross-origin"

    r2 = client.post("/api/v1/auth/login")
    assert r2.headers["Content-Security-Policy"].startswith("default-src 'self'; script-src 'self'")
