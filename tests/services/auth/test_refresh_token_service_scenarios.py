...
async def test_create_user_session_sets_device_ip_and_enforces_limit(monkeypatch):
    ...
    monkeypatch.setattr(rt, "create_access_token", lambda subject, expires_delta: "accesstoken")
    ...
    access, refresh = await rt.create_user_session(db=object(), user=user, request=request)
...
async def test_revoke_all_sessions_calls_crud_with_user(monkeypatch):
    ...
    monkeypatch.setattr("app.crud.revoke_all_user_sessions", fake_revoke_all, raising=True)
    count = await rt.revoke_all_sessions(db=object(), user_id=uuid.uuid4(), except_token_value=None)
    assert count == 3
