...
@pytest.mark.skip(reason="Sentry DSN not configured in template; enable to test init path")
def test_sentry_init_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(sentry_service, "settings", type("S", (), {
        "ENABLE_SENTRY": True,
    ...
    monkeypatch.setattr(sentry_service, "sentry_sdk", type("SDK", (), {
        "init": staticmethod(fake_init),
        "set_tag": staticmethod(lambda *args, **kwargs: None),
        "Hub": type("H", (), {"current": type("C", (), {"client": FakeClient()})}),
    }))
