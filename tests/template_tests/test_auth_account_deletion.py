...
class TestAccountDeletionRequest:
    ...
    def test_request_deletion_success(self, client: TestClient, sync_db_session: Session):
        ...
        with patch('app.services.email.EmailService.is_configured') as mock_configured, \
                patch('app.services.email.EmailService.send_account_deletion_email') as mock_send_email:
        ...
        data = response.json()
        assert data["message"] == "If an account with that email exists, a deletion confirmation link has been sent."
    ...
    def test_request_deletion_user_not_found(self, client: TestClient, sync_db_session: Session):
    ...
    def test_request_deletion_already_requested(self, client: TestClient, sync_db_session: Session):
    ...
    def test_request_deletion_email_not_configured(self, client: TestClient, sync_db_session: Session):
        ...
        with patch('app.services.email.EmailService.is_configured') as mock_configured:
            ...
            response = client.post(
                "/api/v1/auth/request-deletion",
                json={"email": "noconfig@example.com"}
            )

        assert response.status_code == 200
        data = response.json()
        assert data["message"] == "Account deletion service temporarily unavailable. Please try again later."
...
class TestAccountDeletionConfirmation:
    ...
    def test_confirm_deletion_success(self, client: TestClient, sync_db_session: Session):
        ...
        with patch('app.services.email.EmailService.is_configured') as mock_configured, \
                patch('app.services.email.EmailService.verify_deletion_token') as mock_verify:
    ...
    def test_confirm_deletion_invalid_token(self, client: TestClient, sync_db_session: Session):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["message"] == "Invalid or expired deletion token. Please request a new one."
        assert data["deletion_confirmed"] is False

    def test_confirm_deletion_email_not_configured(self, client: TestClient, sync_db_session: Session):
...
class TestAccountDeletionCancellation:
    ...
    def test_cancel_deletion_success(self, client: TestClient, sync_db_session: Session):
    ...
    def test_cancel_deletion_user_not_found(self, client: TestClient, sync_db_session: Session):
    ...
    def test_cancel_deletion_no_request(self, client: TestClient, sync_db_session: Session):
        ...
        response = client.post(
            "/api/v1/auth/cancel-deletion",
            json={"email": "norequest@example.com"}
        )
...
class TestAccountDeletionStatus:
    ...
    def test_deletion_status_not_requested(self, client: TestClient, sync_db_session: Session):
    ...
    def test_deletion_status_requested(self, client: TestClient, sync_db_session: Session):
    ...
    def test_deletion_status_confirmed(self, client: TestClient, sync_db_session: Session):
        """Test status for user who confirmed deletion."""
        scheduled_time = datetime.utcnow(
        ) + timedelta(days=settings.ACCOUNT_DELETION_GRACE_PERIOD_DAYS)
        user = User(
    ...
    def test_deletion_status_user_not_found(self, client: TestClient, sync_db_session: Session):
...
class TestAccountDeletionRateLimiting:
    ...
    def test_request_deletion_rate_limited(self, client: TestClient, sync_db_session: Session):
    ...
    def test_confirm_deletion_rate_limited(self, client: TestClient, sync_db_session: Session):
    ...
    def test_cancel_deletion_rate_limited(self, client: TestClient, sync_db_session: Session):
...
class TestAccountDeletionCeleryTask:
    ...
    def test_permanently_delete_accounts_task_endpoint(self, client: TestClient):
        ...
        with patch('app.services.celery_tasks.permanently_delete_accounts_task.delay') as mock_task:
