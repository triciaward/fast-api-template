...
class TestLoggingConfiguration:
    ...
    def test_file_logging_enabled(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(settings, "ENABLE_FILE_LOGGING", True):
                with patch.object(settings, "LOG_FILE_PATH", str(log_file)):
                    setup_logging()
                    logger = get_app_logger()
    ...
    def test_extra_fields_included(self) -> None:
        ...
        with patch.object(settings, "LOG_INCLUDE_PID", True):
            with patch.object(settings, "LOG_INCLUDE_THREAD", True):
                with patch.object(settings, "LOG_FORMAT", "json"):
                    setup_logging()
                    logger = get_app_logger()
                    with (
