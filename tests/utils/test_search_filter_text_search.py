...
def test_text_search_equals_insensitive_and_not_equals_sensitive():
    ...
    ts_eq = create_text_search("alpha", ["username"], operator=SearchOperator.EQUALS, case_sensitive=False)
    ...
    ts_ne = create_text_search("alpha", ["username"], operator=SearchOperator.NOT_EQUALS, case_sensitive=True)
...
def test_field_filter_not_in_and_in_with_values():
    cfg = SearchFilterConfig(
        filters=[
            create_field_filter("email", FilterOperator.IN, values=["a@b.com", "c@d.com"]),
