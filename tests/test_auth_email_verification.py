...
class TestEmailVerificationEndpoints:
    def test_register_user_with_email_verification(self, client: TestClient) -> None:
        ...
        with patch('app.services.email.email_service.is_configured', return_value=True), \
                patch('app.services.email.email_service.send_verification_email', return_value=True):
    ...
    def test_register_user_email_not_configured(self, client: TestClient) -> None:
        ...
        with patch('app.services.email.email_service.is_configured', return_value=False):
    ...
    def test_login_unverified_user(self, client: TestClient, sync_db_session: Session) -> None:
    ...
    def test_login_verified_user(self, client: TestClient, sync_db_session: Session) -> None:
    ...
    def test_resend_verification_email_success(self, client: TestClient, sync_db_session: Session) -> None:
    ...
    def test_resend_verification_email_already_verified(self, client: TestClient, sync_db_session: Session) -> None:
    ...
    def test_resend_verification_email_not_configured(self, client: TestClient, sync_db_session: Session) -> None:
    ...
    def test_verify_email_success(self, client: TestClient, sync_db_session: Session) -> None:
...
class TestEmailVerificationCRUDOperations:
    def test_get_user_by_verification_token_sync(self, sync_db_session: Session) -> None:
    ...
    def test_update_verification_token_user_not_found(self, sync_db_session: Session) -> None:
...
class TestEmailVerificationIntegration:
    def test_full_email_verification_flow(self, client: TestClient, sync_db_session: Session) -> None:
        ...
        with patch('app.services.email.email_service.is_configured', return_value=True), \
            ...
            assert "Please verify your email before logging in" in login_response.json()[
                "detail"]

            # Step 3: Get the user and manually set verification token
            from app.models.models import User
            user = sync_db_session.query(User).filter(
                User.email == "flow@example.com").first()
