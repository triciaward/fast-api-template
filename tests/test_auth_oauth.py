...
class TestOAuthAuthEndpoints:
    ...
    def test_oauth_login_google_not_configured(self, client: TestClient) -> None:
        ...
        with patch('app.services.oauth.oauth_service.is_provider_configured', return_value=False):
            response = client.post(
                "/api/v1/auth/oauth/login",
    ...
    def test_oauth_login_google_success_new_user(self, client: TestClient, sync_db_session: Session) -> None:
        """Test successful OAuth login for new Google user."""
        with patch('app.services.oauth.settings') as mock_settings, \
                patch('app.services.oauth.oauth_service.verify_google_token') as mock_verify:
    ...
    def test_oauth_login_google_existing_user(self, client: TestClient, sync_db_session: Session) -> None:
    ...
    def test_oauth_login_email_already_registered(self, client: TestClient, sync_db_session: Session) -> None:
        ...
        with patch('app.services.oauth.settings') as mock_settings, \
            ...
            assert response.status_code == 400
            assert "Email already registered with different method" in response.json()[
                "detail"]
    ...
    def test_get_oauth_providers_google_configured(self, client: TestClient) -> None:
        ...
        with patch('app.services.oauth.oauth_service.is_provider_configured') as mock_configured, \
                patch('app.services.oauth.oauth_service.get_oauth_provider_config') as mock_config:
    ...
    def test_get_oauth_providers_both_configured(self, client: TestClient) -> None:
        ...
        with patch('app.services.oauth.oauth_service.is_provider_configured', return_value=True), \
                patch('app.services.oauth.oauth_service.get_oauth_provider_config') as mock_config:
