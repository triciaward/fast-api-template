...
async def list_api_keys(
    ...
    api_keys = await crud_api_key.get_user_api_keys(
        db=db,
        user_id=str(current_user.id),
    ...
    total_count = await crud_api_key.count_user_api_keys(
        db=db,
        user_id=str(current_user.id),
    )
...
async def deactivate_api_key(
    ...
    success = await crud_api_key.deactivate_api_key(
        db=db,
        key_id=key_id,
        user_id=str(current_user.id),
    )
...
async def rotate_api_key(
    ...
    result = await crud_api_key.rotate_api_key(
        db=db,
        key_id=key_id,
        user_id=str(current_user.id),
    )
