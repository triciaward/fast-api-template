...
from app.services.monitoring.audit import log_account_deletion
...
async def request_account_deletion(
    ...
    try:
        ...
        user = await crud_user.get_user_by_email(db, email=deletion_request.email)
        if not user:
        ...
        success = await crud_user.request_account_deletion(db, str(user.id))
        if not success:
            logger.error(
...
async def confirm_account_deletion(
    ...
    try:
        ...
        # Confirm deletion
        success = await crud_user.confirm_account_deletion(db, user_id)
        if not success:
...
async def cancel_account_deletion(
    ...
    try:
        ...
        # Cancel deletion
        success = await crud_user.cancel_account_deletion(db, str(user.id))
        if not success:
            logger.error(
