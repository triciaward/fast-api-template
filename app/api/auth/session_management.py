...
from app.crud.auth import refresh_token as crud_refresh_token
...
from app.services.monitoring.audit import log_logout
...
async def logout(
    ...
    try:
        ...
        if refresh_token_value:
            ...
            if db_token:
                user = await crud_user.get_user_by_id(db, str(db_token.user_id))
...
async def get_user_sessions(
    ...
    try:
        ...
        from app.services.auth.refresh_token import get_refresh_token_from_cookie
...
async def revoke_all_sessions(
    ...
    db_user = await crud_user.get_user_by_id(db, str(current_user.id))
    if not db_user:
