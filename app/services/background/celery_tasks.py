...
@celery_app.task(name="app.services.celery_tasks.send_email_task")  # type: ignore[misc]
...
@celery_app.task(name="app.services.celery_tasks.process_data_task")  # type: ignore[misc]
def process_data_task(data: list[dict[str, Any]]) -> dict[str, Any]:
    time.sleep(1)
    return {"status": "processed", "count": len(data)}


@celery_app.task(name="app.services.celery_tasks.cleanup_task")  # type: ignore[misc]
def cleanup_task() -> dict[str, Any]:
    time.sleep(1)
    return {"status": "cleanup complete"}


@celery_app.task(name="app.services.celery_tasks.long_running_task")  # type: ignore[misc]
...
@celery_app.task(name="app.services.celery_tasks.periodic_health_check")  # type: ignore[misc]
def periodic_health_check() -> dict[str, Any]:
    time.sleep(1)
    return {"status": "healthy"}


@celery_app.task(name="app.services.celery_tasks.permanently_delete_accounts_task")  # type: ignore[misc]
