...
class Settings(BaseSettings):
    ...
    # Database - CRITICAL: Must be set in production via environment variable
    DATABASE_URL: str = (
    ...
    _SECRET_KEY_PROD_ERROR = (
        "SECRET_KEY must be set to a secure random value in production! "
        "Use 'openssl rand -hex 32' to generate a secure key."
    )
    _SECRET_KEY_LENGTH_ERROR = "SECRET_KEY must be at least 32 characters in production"
    _DATABASE_URL_PROD_ERROR = (
        "DATABASE_URL must not contain default/development passwords in production!"
    )
    ...
    def validate_secret_key(cls, v: str) -> str:
        ...
        if v in [
            ...
            if env.lower() in ["production", "prod", "staging"]:
                raise ValueError(cls._SECRET_KEY_PROD_ERROR)

        # Check minimum length for security
        if len(v) < 32:
            env = os.getenv("ENVIRONMENT", "development")
            if env.lower() in ["production", "prod", "staging"]:
                raise ValueError(cls._SECRET_KEY_LENGTH_ERROR)
    ...
    def validate_database_url(cls, v: str) -> str:
        """Validate that DATABASE_URL doesn't contain weak passwords in production."""
        if "dev_password" in v:
            env = os.getenv("ENVIRONMENT", "development")
            if env.lower() in ["production", "prod", "staging"]:
                raise ValueError(cls._DATABASE_URL_PROD_ERROR)
        return v
